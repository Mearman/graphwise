 ...
 * BASE generator core.
 *
 * Contains the exploration loop as a generator function that yields GraphOp
...
export function* baseCore<N extends NodeData, E extends EdgeData>(
...
): Generator<GraphOp, ExplorationResult, GraphOpResponse<N, E>> {
	...
	let termination: ExplorationStats["termination"] = "exhausted";

	const limits: ExplorationLimits = { maxIterations, maxNodes, maxPaths };

	// Main exploration loop — limits are checked at the start of each iteration
	for (;;) {
		...
		// All queues empty — exploration exhausted
		if (activeFrontier < 0) {
...
function buildPriorityContext<N extends NodeData, E extends EdgeData>(
	...
	discoveredPaths: readonly ExplorationPath[],
	iteration: number,
	degree: number,
