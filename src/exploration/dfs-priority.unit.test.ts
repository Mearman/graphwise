...
describe("dfsPriority exploration", () => {
	it("returns empty result for no seeds", () => {
		const graph = createTestGraph();
		const result = dfsPriority(graph, []);
	...
	it("returns a result object with correct structure", () => {
		...
		const result = dfsPriority(graph, seeds);
	...
	it("finds paths between seeds on a linear chain", () => {
		const graph = createTestGraph();
		const seeds: Seed[] = [{ id: "A" }, { id: "E" }];
	...
	it("handles disconnected seeds", () => {
		...
		const result = dfsPriority(graph, [{ id: "A" }, { id: "B" }]);
	...
	it("respects maxNodes limit", () => {
		...
		const result = dfsPriority(graph, seeds, { maxNodes: 2 });
...
describe("dfsPriorityAsync exploration", () => {
