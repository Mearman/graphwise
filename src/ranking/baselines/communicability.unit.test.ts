...
import { communicability } from "./communicability";

describe("communicability baseline", () => {
	it("returns empty array for no paths", () => {
		const graph = AdjacencyMapGraph.undirected<KGNode>();
		const result = communicability(graph, []);

		expect(result.paths).toHaveLength(0);
		expect(result.method).toBe("communicability");
	});
	...
	it("ranks single path", () => {
		...
		const result = communicability(graph, paths);
