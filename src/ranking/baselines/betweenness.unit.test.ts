...
describe("betweenness baseline", () => {
	...
	it("ranks single path", () => {
		...
		const result = betweenness(graph, paths);
	...
	it("ranks multiple paths by betweenness sum", () => {
		const graph = AdjacencyMapGraph.undirected<TestNode, TestEdge>();
		const nodes = ["A", "B", "C", "D"];
